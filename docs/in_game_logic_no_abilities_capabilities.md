...
| `Completed` | Gameplay runtime skeleton | Added `gameplay` runtime structure to `Game` with player/shared zone buckets and policy modifiers. |
...
| `Pending` | Gameplay projection fields | Add stable read projection (`turn`, `phase`, `timing`, `priority`) sourced from the intent-driven runtime state. |
